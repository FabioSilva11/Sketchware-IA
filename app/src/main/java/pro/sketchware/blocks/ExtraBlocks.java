...
public class ExtraBlocks {
    ...
    public static void extraBlocks(ArrayList<HashMap<String, Object>> arrayList) {
        ...
        arrayList.add(addBlock("fusedLocationStart", " ", "", "{ _%1$s_location_request = LocationRequest.create().setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY).setInterval(Math.max(1000L, (long)%2$s)).setFastestInterval(Math.max(500L, (long)%3$s)); _%1$s_start_location_updates(); }", "#607d8b", "%m.fusedlocationmanager start updates interval %d ms minimum %d ms"));
        arrayList.add(addBlock("fusedLocationStop", " ", "", "_%1$s_stop_location_updates();", "#607d8b", "%m.fusedlocationmanager stop updates"));
