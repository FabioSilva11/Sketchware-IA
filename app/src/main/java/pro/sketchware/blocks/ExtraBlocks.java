...
public class ExtraBlocks {
    ...
    public static void extraBlocks(ArrayList<HashMap<String, Object>> arrayList) {
        ...
        arrayList.add(addBlock("bannerAdViewLoadAdaptive", " ", "", "{ int _adWidth = (int)(getResources().getDisplayMetrics().widthPixels / getResources().getDisplayMetrics().density); %1$s.setAdSize(AdSize.getLargeAnchoredAdaptiveBannerAdSize(this, _adWidth)); %1$s.loadAd(new AdRequest.Builder().build()); }", "#4a6cd4", "%m.adview load large adaptive banner"));
        arrayList.add(addBlock("admobRequestConsent", " ", "", "{ com.google.android.ump.ConsentRequestParameters _consentParams = new com.google.android.ump.ConsentRequestParameters.Builder().build(); com.google.android.ump.ConsentInformation _consentInfo = com.google.android.ump.UserMessagingPlatform.getConsentInformation(this); _consentInfo.requestConsentInfoUpdate(this, _consentParams, () -> com.google.android.ump.UserMessagingPlatform.loadAndShowConsentFormIfRequired(this, _formError -> { if (_formError != null) SketchwareUtil.showMessage(getApplicationContext(), _formError.getMessage()); }), _requestError -> SketchwareUtil.showMessage(getApplicationContext(), _requestError.getMessage())); }", "#4a6cd4", "AdMob request consent update and show required form"));
        arrayList.add(addBlock("admobShowPrivacyOptions", " ", "", "com.google.android.ump.UserMessagingPlatform.showPrivacyOptionsForm(this, _formError -> { if (_formError != null) SketchwareUtil.showMessage(getApplicationContext(), _formError.getMessage()); });", "#4a6cd4", "AdMob show privacy options"));
        //OtpView Blocks
