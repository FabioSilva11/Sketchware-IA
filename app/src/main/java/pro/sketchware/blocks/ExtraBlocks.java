...
public class ExtraBlocks {
    ...
    public static void extraBlocks(ArrayList<HashMap<String, Object>> arrayList) {
        ...
        // Background, system and identity managers
        arrayList.add(addBlock("workManagerEnqueueOnce", " ", "", "try { Class<? extends ListenableWorker> _workerClass = (Class<? extends ListenableWorker>) Class.forName(%2$s); OneTimeWorkRequest _workRequest = new OneTimeWorkRequest.Builder(_workerClass).setInitialDelay((long)%3$s, TimeUnit.MINUTES).addTag(%4$s).build(); %1$s.enqueue(_workRequest); } catch (Exception _e) { SketchwareUtil.showMessage(getApplicationContext(), _e.getMessage()); }", "#607d8b", "%m.workmanager enqueue worker class %s after %d minutes tag %s"));
        arrayList.add(addBlock("workManagerEnqueuePeriodic", " ", "", "try { Class<? extends ListenableWorker> _workerClass = (Class<? extends ListenableWorker>) Class.forName(%2$s); PeriodicWorkRequest _workRequest = new PeriodicWorkRequest.Builder(_workerClass, Math.max(15L, (long)%3$s), TimeUnit.MINUTES).addTag(%4$s).build(); %1$s.enqueue(_workRequest); } catch (Exception _e) { SketchwareUtil.showMessage(getApplicationContext(), _e.getMessage()); }", "#607d8b", "%m.workmanager enqueue periodic worker class %s every %d minutes tag %s"));
        arrayList.add(addBlock("workManagerCancelTag", " ", "", "%1$s.cancelAllWorkByTag(%2$s);", "#607d8b", "%m.workmanager cancel work tag %s"));
        arrayList.add(addBlock("workManagerCancelAll", " ", "", "%1$s.cancelAllWork();", "#607d8b", "%m.workmanager cancel all work"));
        arrayList.add(addBlock("alarmManagerCanExact", "b", "", "Build.VERSION.SDK_INT < 31 || %1$s.canScheduleExactAlarms()", "#607d8b", "%m.alarmmanager can schedule exact alarms"));
        arrayList.add(addBlock("alarmManagerRequestExactAccess", " ", "", "if (Build.VERSION.SDK_INT >= 31 && !%1$s.canScheduleExactAlarms()) { startActivity(new Intent(android.provider.Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM, android.net.Uri.parse(\"package:\" + getPackageName()))); }", "#607d8b", "%m.alarmmanager request exact alarm access"));
        arrayList.add(addBlock("alarmManagerScheduleActivity", " ", "", "{ Intent _alarmIntent = new Intent(getApplicationContext(), getClass()); PendingIntent _alarmPendingIntent = PendingIntent.getActivity(getApplicationContext(), (int)%2$s, _alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE); if (Build.VERSION.SDK_INT < 31 || %1$s.canScheduleExactAlarms()) { %1$s.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, (long)%3$s, _alarmPendingIntent); } }", "#607d8b", "%m.alarmmanager open this activity request code %d at timestamp %d"));
        arrayList.add(addBlock("alarmManagerCancel", " ", "", "{ Intent _alarmIntent = new Intent(getApplicationContext(), getClass()); PendingIntent _alarmPendingIntent = PendingIntent.getActivity(getApplicationContext(), (int)%2$s, _alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE); %1$s.cancel(_alarmPendingIntent); }", "#607d8b", "%m.alarmmanager cancel request code %d"));
        arrayList.add(addBlock("biometricManagerCanAuthenticate", "b", "", "_%1$s_manager.canAuthenticate(BiometricManager.Authenticators.BIOMETRIC_STRONG) == BiometricManager.BIOMETRIC_SUCCESS", "#607d8b", "%m.biometricmanager can authenticate"));
        arrayList.add(addBlock("biometricManagerAuthenticate", " ", "", "%1$s.authenticate(new BiometricPrompt.PromptInfo.Builder().setTitle(%2$s).setSubtitle(%3$s).setNegativeButtonText(%4$s).build());", "#607d8b", "%m.biometricmanager authenticate title %s subtitle %s cancel text %s"));
        // Generate the legacy LocationRequest factory API because Sketchware projects may
        // compile against older play-services-location JARs. These deprecated methods are
        // also retained by current Play services, so the generated source works with both.
        arrayList.add(addBlock("fusedLocationStart", " ", "", "{ LocationRequest _locationRequest = LocationRequest.create().setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY).setInterval(Math.max(1000L, (long)%2$s)).setFastestInterval(Math.max(500L, (long)%3$s)); %1$s.requestLocationUpdates(_locationRequest, _%1$s_location_callback, Looper.getMainLooper()); }", "#607d8b", "%m.fusedlocationmanager start updates interval %d ms minimum %d ms"));
        arrayList.add(addBlock("fusedLocationStop", " ", "", "%1$s.removeLocationUpdates(_%1$s_location_callback);", "#607d8b", "%m.fusedlocationmanager stop updates"));
        arrayList.add(addBlock("bannerAdViewLoadAdaptive", " ", "", "{ int _adWidth = (int)(getResources().getDisplayMetrics().widthPixels / getResources().getDisplayMetrics().density); %1$s.setAdSize(AdSize.getCurrentOrientationAnchoredAdaptiveBannerAdSize(this, _adWidth)); %1$s.loadAd(new AdRequest.Builder().build()); }", "#4a6cd4", "%m.adview load adaptive banner"));
        arrayList.add(addBlock("admobRequestConsent", " ", "", "{ com.google.android.ump.ConsentRequestParameters _consentParams = new com.google.android.ump.ConsentRequestParameters.Builder().build(); com.google.android.ump.ConsentInformation _consentInfo = com.google.android.ump.UserMessagingPlatform.getConsentInformation(this); _consentInfo.requestConsentInfoUpdate(this, _consentParams, new com.google.android.ump.ConsentInformation.OnConsentInfoUpdateSuccessListener() { @Override public void onConsentInfoUpdateSuccess() { com.google.android.ump.UserMessagingPlatform.loadAndShowConsentFormIfRequired(getActivity(), new com.google.android.ump.ConsentForm.OnConsentFormDismissedListener() { @Override public void onConsentFormDismissed(com.google.android.ump.FormError _formError) { if (_formError != null) SketchwareUtil.showMessage(getApplicationContext(), _formError.getMessage()); } }); } }, new com.google.android.ump.ConsentInformation.OnConsentInfoUpdateFailureListener() { @Override public void onConsentInfoUpdateFailure(com.google.android.ump.FormError _requestError) { SketchwareUtil.showMessage(getApplicationContext(), _requestError.getMessage()); } }); }", "#4a6cd4", "AdMob request consent update and show required form"));
        arrayList.add(addBlock("admobShowPrivacyOptions", " ", "", "com.google.android.ump.UserMessagingPlatform.showPrivacyOptionsForm(this, new com.google.android.ump.ConsentForm.OnConsentFormDismissedListener() { @Override public void onConsentFormDismissed(com.google.android.ump.FormError _formError) { if (_formError != null) SketchwareUtil.showMessage(getApplicationContext(), _formError.getMessage()); } });", "#4a6cd4", "AdMob show privacy options"));
        //OtpView Blocks
