...
public class ExtraBlocks {
    ...
    public static void extraBlocks(ArrayList<HashMap<String, Object>> arrayList) {
        ...
        arrayList.add(addBlock("fusedLocationStart", " ", "", "{ LocationRequest _locationRequest = new LocationRequest.Builder(Priority.PRIORITY_HIGH_ACCURACY, Math.max(1000L, (long)%2$s)).setMinUpdateIntervalMillis(Math.max(500L, (long)%3$s)).build(); %1$s.requestLocationUpdates(_locationRequest, _%1$s_location_callback, Looper.getMainLooper()); }", "#607d8b", "%m.fusedlocationmanager start updates interval %d ms minimum %d ms"));
